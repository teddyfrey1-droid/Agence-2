...
function SidebarOverlay({ onClose, pathname }: { onClose: () => void; pathname: string }) {
  useEffect(() => {
  ...
  return (
    <>
      ...
      >
        <div className="flex h-14 flex-shrink-0 items-center justify-between border-b border-stone-100 px-4 dark:border-stone-700/50">
          <Logo size="sm" />
          <button
            onClick={onClose}
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4">
          {navGroups.map((group, gi) => (
            <div key={group.label || "main"} className={cn(gi > 0 && "mt-5")}>
              {group.label && (
                      ...
                      <Link
                        ...
                        className={cn(
                          ...
                          isActive
                            ...
                            : "text-anthracite-600 hover:bg-stone-100/70 hover:text-anthracite-900 dark:text-stone-400 dark:hover:bg-anthracite-800 dark:hover:text-stone-100"
                        )}
                      >
                        <svg
                          className={cn("h-[18px] w-[18px] flex-shrink-0", isActive ? "text-brand-600 dark:text-brand-400" : "text-stone-400 dark:text-stone-500")}
        ...
        <div className="flex-shrink-0 border-t border-stone-100 p-4 dark:border-stone-700/50">
          <Link
            href="/"
            onClick={onClose}
  ...
  return (
    <>
      ...
      {isOpen && portalRef.current && createPortal(
        <SidebarOverlay onClose={() => setIsOpen(false)} pathname={pathname} />,
        portalRef.current
      )}
    </>
  );
