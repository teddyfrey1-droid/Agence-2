  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-anthracite-900">Demandes de recherche</h1>
      ...
      {items.length === 0 ? (
        <EmptyState
          ...
          description="Les demandes de recherche soumises via le site ou créées manuellement apparaîtront ici."
        />
      ) : (
              ...
              <tbody className="divide-y divide-stone-100">
                {items.map((request) => (
                  <tr key={request.id} className="hover:bg-stone-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {request.contact ? (
                        <span className="text-anthracite-800">
                          {request.contact.firstName} {request.contact.lastName}
                          {request.contact.company && (
                            <span className="block text-xs text-stone-400">{request.contact.company}</span>
                          )}
                        </span>
                      ) : (
