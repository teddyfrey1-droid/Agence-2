...
// ── Transaction types — short labels for touch UI ──
const TRANSACTION_TYPES = [
  {
    value: "LOCATION",
    label: "Location",
    sublabel: "Bail commercial",
    icon: "M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25",
  },
  {
    value: "VENTE",
    label: "Vente mur",
    sublabel: "Murs commerciaux",
      ...
      <form onSubmit={handleSubmit} onChange={() => setIsDirty(true)} className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            <div>
              <label className="mb-3 block text-sm font-medium text-anthracite-700 dark:text-stone-300">
                Type de transaction
              </label>
                ...
                {TRANSACTION_TYPES.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center gap-2.5 rounded-2xl border-2 px-3 py-5 text-center transition-all active:scale-95 ${
                        active
                          ? "border-brand-500 bg-brand-500 text-white shadow-md"
                          : "border-stone-100 bg-white text-anthracite-700 hover:border-brand-200 hover:bg-brand-50 dark:border-stone-700 dark:bg-anthracite-800 dark:text-stone-300 dark:hover:border-brand-700"
                      }`}
                    >
                      <svg
                        className={`h-7 w-7 flex-shrink-0 ${active ? "text-white" : "text-brand-500 dark:text-brand-400"}`}
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        strokeWidth={1.5}
                      ...
                      <div>
                        ...
                        <p className={`mt-0.5 text-[10px] leading-tight ${active ? "text-white/80" : "text-stone-400 dark:text-stone-500"}`}>
                          {opt.sublabel}
                        </p>
                      </div>
                    </button>
