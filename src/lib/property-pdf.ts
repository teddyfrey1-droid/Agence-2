...
export interface PropertyPdfPayload {
...
function vatRegimeLabel(regime: string | null | undefined): string {
  if (regime === "NON_SOUMIS") return "Non soumis à TVA";
  return "HT";
}
...
/**
 * Fetches the property + agency payload, then renders the commercial fiche
 * client-side with jsPDF and triggers the browser download. Shared between
 * the desktop button and the mobile bottom-bar so they stay in lockstep.
 */
export async function generatePropertyFichePdf(propertyId: string): Promise<void> {
  ...
  if (res.status === 404) throw new Error("Bien introuvable.");
  if (!res.ok) throw new Error("Impossible de récupérer la fiche du bien.");
  const payload = (await res.json().catch(() => null)) as PropertyPdfPayload | null;
  if (!payload || !payload.property || !payload.agency) {
    throw new Error("Réponse serveur invalide.");
  }
  const { property, agency } = payload;
  ...
  const priceLabel = isLocation ? "Loyer mensuel" : "Prix";
  const priceValue = isLocation
    ? `${fmtEUR(property.rentMonthly)} ${vatRegimeLabel(property.rentVatRegime)}`.trim()
    : fmtEUR(property.price);
  ...
  if (isLocation) {
    const vatTag = vatRegimeLabel(property.rentVatRegime);
    if (property.rentMonthly) finRows.push(["Loyer mensuel", `${fmtEUR(property.rentMonthly)} ${vatTag}`.trim()]);
    if (property.rentYearly) finRows.push(["Loyer annuel", `${fmtEUR(property.rentYearly)} ${vatTag}`.trim()]);
    if (property.charges) {
      finRows.push(["Charges mensuelles", `${fmtEUR(property.charges)} (le loyer est hors charges)`]);
    } else {
      finRows.push(["Charges mensuelles", "Le loyer est hors charges (HC)"]);
    }
